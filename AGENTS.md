...
- 文档地图：docs/README.md
- 调研篇（设计依据与出处）：docs/research/harness-best-practices.zh.md
- 实施篇（完整方案与路线图）：docs/solution/vibe-coding-harness-plan.zh.md
...
1. **状态行**：每份文档头部必须有 `> status:` 行，取值：`生效` / `已过时 → 看xxx（链接）` / `已废弃（原因）`。文档被新版本取代时，旧文档**原地保留**、状态改"已过时"并加指向链接——**禁止搬进归档文件夹**（一个知识一个家）。
...
4. **防篡改**：git 哈希链（自动留痕）+ 远端禁止改写历史（一次性设置）+ `scripts/check-docs.sh` 提交时自动体检（链接 / 双语配对 / 新鲜度）。

## Bilingual Rules（双语规则）

1. 命名：中文 `*.zh.md`，英文 `*.en.md`，成对存放于同一目录。
2. **中文为权威版本**：冲突时以中文为准，英文是翻译。
3. 改动中文权威文档时，必须同步更新英文版；来不及就先在英文版头部标注 `> sync: 落后于中文（截至日期）`，并尽快补齐。
4. 内部活文档（决策记录、模板、适配卡、计划）默认仅中文；需要英文时按需补译。全量双语化见终极方案目录 B9。
