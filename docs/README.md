...
1. **新成员/首次接触**：[实施篇](solution/vibe-coding-harness-plan.md) 第一、二部分建立认知 → 跳到第三部分自己所在的领域章节；
2. **想知道"为什么这么设计"**：[调研篇](research/harness-best-practices.md)（含全部参考文章链接与逐篇要点）；
3. **动手搭建**：复制 [templates/](templates/README.md) 到目标仓库 → 按自己领域填写[适配卡](adapters/README.md) → 按[实施篇第四部分](solution/vibe-coding-harness-plan.md)路线图推进。
