...
- **Revision (2026-09-15, same day)**: hosting priority is fixed as **Gitea → GitHub → GitLab** (Gitea first, per team requirement). This repository now ships all three CI wirings out of the box (`.gitea/workflows/docs-check.yml`, `.github/workflows/docs-check.yml`, `.gitlab-ci.yml`), so the doc gates run identically on whichever platform hosts or mirrors the repo. Gitea deployment itself remains gated by B1.
