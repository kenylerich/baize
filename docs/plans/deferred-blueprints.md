...
|---|---|---|---|---|
| B1 | Docker Gitea 评审门禁（分支保护 + PR + CI + Approve）——**三平台中的优先项** | 出现第二个提交主体：来了第二个同事，或 Agent 提交量大到人肉把关不过来 | `infra/gitea/`（部署包 + 启用四步）；决策记录 D3/D10；跨平台闸门契约 `docs/solution/quality-gates.md`；本仓库已内置三平台 CI 配置（.gitea/.github/.gitlab-ci.yml） | 未启用（部署待触发；三平台 CI 配置已就绪） |
