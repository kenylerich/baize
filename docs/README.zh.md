...
└── plans/        # 落地计划、进度、终极方案目录（deferred-blueprints.zh.md）：活文档
```
...
3. **动手搭建**：复制 [templates/](templates/README.zh.md) 到目标仓库 → 按自己领域填写[适配卡](adapters/README.zh.md) → 按[实施篇第四部分](solution/vibe-coding-harness-plan.zh.md)路线图推进。
...
- **讨论入库**：聊出结论当场落盘，"以后再说"的必须登记进 [plans/deferred-blueprints.zh.md](plans/deferred-blueprints.zh.md)——完整规则见根目录 [AGENTS.md](../AGENTS.md)；
- **双语**：英文（默认文件名）为权威版本；改动权威文档必须同步 `.zh.md` 中文翻译，来不及先标注"落后"；由 `scripts/check-docs.mjs` 机械检查。双语覆盖现状见 [plans/translation-status.zh.md](plans/translation-status.zh.md)；
